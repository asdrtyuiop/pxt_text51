...
namespace artecrobo {
    ...
    // let speedM1 = 1023;
    // let speedM2 = 1023;
    // let state1 = DCmotion1.Brake;
    // // Move DC motor
    // //% blockId=artec_move_dc_motor group="Motor"
    // //% block="DC motor %_connector| motion: %_motion"
    // export function moveDCMotor1(_connector: connectorDCMotor1, _motion: DCmotion1): void {
    //     switch(_motion) {
    //         case DCmotion1.Forward:
    //             /*
    //                 Move Forward
    //                 M1:P8 = speed, P12 = 0
    //                 M2:P0 = speed, P16 = 0
    //             */
    //             if (_connector == connectorDCMotor1.M1) {
    ...
    // export function setSpeedDCMotor1(_connector: connectorDCMotor1, _speed: number): void {
    //     if (_speed < 0)     { _speed = 0; }
    ...
    //         speedM2 = 1023 - _speed;
    //     }
    //     if (state1 == DCmotion1.Forward || state1 == DCmotion1.Backward) {
    //         moveDCMotor1(_connector, state1);
    //     }
    // }
    ...
    pins.servoWritePin(AnalogPin.P16, angleP16);
    ...
    export function moveServoMotorMax(_connector: connectorServoMotor, _angle: number): void {
        ...
        switch (_connector) {
            ...
            case connectorServoMotor.P15:
                ...
                angleP15 = _angle;
                break;
            case connectorServoMotor.P16:
                pins.servoWritePin(AnalogPin.P16, _angle);
                angleP16 = _angle;
                break;
            default:
                break;
    ...
    export function moveServoMotor(_connector: connectorServoMotor, _angle: number, _speed: number): void {
        ...
        switch (_connector) {
            ...
            case connectorServoMotor.P15:
                ...
                break;
            case connectorServoMotor.P16:
                moveservo(AnalogPin.P16, angleP16, _angle, _speed);
                angleP16 = _angle;
                break;
            default:
                break;
     ...
     * @param angle16 ServoMotor Angle P16
	 */
    //% weight=84 group="Motor"
    //% blockId=artec_async_move_servo_motor
    //% block="move servo synchronously | speed: %_speed| P13 (degrees): %_angle13| P14 (degrees): %_angle14 |P15 (degrees): %_angle15| P16 (degrees): %_angle16"
    //% _speed.min=1 _speed.max=20
    //% _angle13.min=0 _angle13.max=180
    //% _angle14.min=0 _angle14.max=180
    //% _angle15.min=0 _angle15.max=180
    //% _angle16.min=0 _angle16.max=180
    export function AsyncMoveServoMotor(_speed: number, _angle13: number, _angle14: number, _angle15: number, _angle16: number): void {
        if (_speed < 0) { _speed = 0; }
        if (_speed > 20) { _speed = 20; }
        ...
        if (_angle16 < 0) { _angle16 = 0; }
        if (_angle16 > 180) { _angle16 = 180; }
        const interval = Math.abs(_speed - 20) + 3;
        // サーボモーターを動かす方向
        let dirP13 = 1;
        ...
        const diffP16 = Math.abs(angleP16 - _angle16);    // 変化量
        let maxData = Math.max(diffP13, diffP14, diffP15);
        maxData = Math.max(maxData, diffP16);
        ...
        let divideP16 = 0;
        if (diffP16 != 0) {
            divideP16 = maxData / diffP16;  // 1度変化させる間隔
        }
        ...
        if (diffP16 != 0) pins.servoWritePin(AnalogPin.P16, angleP16);
    }
